...
class RouteResource extends JsonResource
{
    ...
    {
        $travel_time = Carbon::parse($this->arrival_time)->diff($this->departure_time)->format('%H:%I:%S');
        $from_place = Place::query()->find($this->from_place_id);
        $to_place = Place::query()->find($this->to_place_id);
        $number_searches = RouteSearch::query()->where('schedule_id', $this->id)->count();
        return [
            'number_history_select' => $number_searches,
            'schedules' => [
                'id' => $this->id,
                'line' => $this->line,
